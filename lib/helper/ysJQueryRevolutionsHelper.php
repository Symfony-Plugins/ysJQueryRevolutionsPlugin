...
$jquery_path = sfConfig::get('app_ys_jquery_path', null);
if($jquery_path == null){
    $jquery_path = core_get_jquery_path();
}
sfContext::getInstance()
           ->getResponse()
           ->addJavascript($jquery_path, 'first');
/*
 || AJAX------------------------------------------------------------------------
 */
...
function jquery_sintax_builder($selector , $events = 'ready' , $args ="function(){return false;}", $unescapeId = true, $accesors = '', $addSeparator = true){
    ...
    if(!is_null($selector)){
      if(is_array($selector) && sizeof($selector) > 1){
        if (strtolower(trim($selector[0])) == 'this')
        $unescapeId = false;
        if($unescapeId){
          return sprintf("jQuery('%s', %s).%s(%s)%s%s",$selector[0],$selector[1],$events,$args,$accesors, $separator);
        }else{
          return sprintf("jQuery(%s, %s).%s(%s)%s%s",$selector[0],$selector[1],$events,$args,$accesors, $separator);
        }
      }else{
        if (strtolower(trim($selector)) == 'this')
        $unescapeId = false;
        if($unescapeId){
          return sprintf("jQuery('%s').%s(%s)%s%s",(string) $selector,$events,$args,$accesors, $separator);
        }else{
          return sprintf("jQuery(%s).%s(%s)%s%s",(string) $selector,$events,$args,$accesors, $separator);
        }
      }
    }else{
      return sprintf("jQuery.%s(%s)%s%s",$events,$args,$accesors,$separator);
    }
}
...
function jquery_set($selector , $events , $args = null, $unescapeId = true, $accesors = '', $addSeparator = false){
  //$args2  = (is_array($args) ? json_encode($args) : $args );
...
function core_get_jquery_path(){
    $os_separator = sfConfig::get('path_separattor', '/');
    return $os_separator .sfConfig::get('app_ys_jquery_web_dir', 'ysJQueryRevolutionsPlugin')
           . $os_separator . sfConfig::get('ys_jquery_js_folder', 'js')
           . $os_separator . sfConfig::get('app_ys_jquery_js_dir', 'jquery')
           . $os_separator . sfConfig::get('app_ys_jquery_core', 'jquery-1.3.2.min.js');
}
...
function return_jquery($configurations, $sintax){
  ...
  if(is_array($configurations) && isset($configurations['listener']) ){
      ...
      if(isset($listener['event']) && isset($listener['selector'])){
        $value = add_jquery_support($listener['selector'],$listener['event'],like_function($value));
      }
  }
  return $value;
}
