...
function jquery_sintax_builder($selector , $events = 'ready' , $args ="function(){return false;}", $unescapeId = true, $accesors = '', $addSeparator = true){
    ...
    if(!is_null($selector)){
      if(is_array($selector) && sizeof($selector) > 1){
        ...
        $unescapeId = false;
        if($unescapeId){
          return sprintf("$('%s', %s).%s(%s)%s%s",$selector[0],$selector[1],$events,$args,$accesors, $separator);
        }else{
          return sprintf("$(%s, %s).%s(%s)%s%s",$selector[0],$selector[1],$events,$args,$accesors, $separator);
        }
      }else{
        ...
        $unescapeId = false;
        if($unescapeId){
          return sprintf("$('%s').%s(%s)%s%s",(string) $selector,$events,$args,$accesors, $separator);
        }else{
          return sprintf("$(%s).%s(%s)%s%s",(string) $selector,$events,$args,$accesors, $separator);
        }
      }
    }else{
      return sprintf("$.%s(%s)%s%s",$events,$args,$accesors,$separator);
    }
}
